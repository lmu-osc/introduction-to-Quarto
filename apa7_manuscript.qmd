...
3. Declare the language of the document by setting e.g. `lang: en` in the YAML header (see the [Quarto documentation on document language for more information](https://quarto.org/docs/authoring/language.html))
...
For including images, the `apaquarto` extension requires the use of code blocks, which need to have a label that starts with `fig-`. Captions are in [title case](https://apastyle.apa.org/style-grammar-guidelines/capitalization/title-case):
...
```{{r}}
#| label: fig-flower
#| fig-cap: This Image Depicts a Flower.
...
For more information on how to work with figures, head over to the [Quarto documentation on figures](https://quarto.org/docs/authoring/figures.html#computations).
...
If you are interested in all the options provided by the `apaquarto` extension (e.g., masked citations for anonymous peer review), have a look at the [apaquarto documentation](https://wjschne.github.io/apaquarto/options.html).
